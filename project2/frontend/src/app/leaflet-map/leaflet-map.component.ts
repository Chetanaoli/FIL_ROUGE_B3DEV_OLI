...
export class LeafletMapComponent implements OnInit, AfterViewInit, OnDestroy  {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        if (!savedLocationArray) {
          ...
          this.map.setView(savedLocationArray, 9);
          this.fetchWeatherData(savedLocationArray[0], savedLocationArray[1]);
        }
  ...
  private fetchWeatherData(lat: number, long: number): void {
    const url = `https://api.weatherapi.com/v1/current.json?key=d0b4be268504482db2c184600232008&q=${lat},${long}`;
    fetch(url)
